...
def train(model,device,train_loader,optim,epoch):
    ...
    for batch, (data,targets) in enumerate(pbar):
        ...
        epoch_acc += ((preds == target).sum() / len(target))
        correct += (preds==target).sum().item()
        processed += len(target)
        pbar.set_description(f"Loss = {loss.item()},batch_idx = {batch}, accuracy = {100* correct/processed}")
...
def test(model,device,test_loader,epoch):
    ...
    with torch.no_grad():
        for batch, (data, target) in enumerate(test_loader):
            ...
            outputs = model(data)
            loss = F.nll_loss(outputs,target,reduction="sum")
            test_loss += loss
    ...
    print("\n Test set Avg Loss = {:.4f}, Accuracy {}/{},({:.2f}%)\n".format(test_loss,correct,processed,100*(correct/processed)))
